...
export async function hydratePortfolio() {
  try {
    ...
    const { data, error } = await supabase.from('portfolio_projects').select('slug,title,description,tags,thumbnail_path,cover_path,featured,published,content,sort_order').eq('published', true).order('sort_order', { ascending: true });
